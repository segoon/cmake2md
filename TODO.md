...
1. **`@ingroup` takes a name from the next line.** `_NAME_RE` in
   `doc_parser.py` is matched with `re.DOTALL`, so the name search runs past
   the end of the line and even past a blank line. A tag whose name was
   forgotten silently eats the first word of the prose below it:

   ```cmake
   # @ingroup
   # Build targets is where this belongs.
   ```

   parses as group `Build` with the description `targets is where this
   belongs.`, and the run succeeds. With a `@defgroup` in the project the
   group check catches it, but blames a name the author never wrote. Stop the
   name at the line end, and let a tag alone on its line fall through to the
   existing "requires a name" error.

2. **`output = "-"` in the config file writes a file called `-`.** `output`
   and `json` are in `config.PATH_KEYS`, so the `-` that means stdout is
   resolved against the config file like any other path. It also slips past
   the `--check` guard in `validate_args`, which looks for the literal `-`.
   `_against()` should hand `-` back untouched; the constant lives in `cli`
   and should be shared rather than spelled twice.

3. **Every diagnostic on an `option()` or `set(... CACHE ...)` comment prints
   twice.** The same comment node is enriched once as a `Command` and once as
   a `Variable`, and `enrich()` reports for both, differing only in the kind
   it names. Visible under `--no-strict`, which is the mode meant for reading
   warnings. Parse each distinct comment block once and share the result, or
   report once per `(filepath, comments_line, message)`.

4. **A source named twice is parsed twice.** `collect_sources` does not
...
5. **Duplicate definitions are silently overwritten.** A second `@brief` or
...
6. **`@file` outside a comment block of its own does nothing, quietly.** On a
...
7. **`symbol_link` never links anything in the built-in reference.**
...
8. **An I/O failure is a traceback.** `write_output` and `read_ignore_file`
...
9. **`check = true` in the config file cannot be turned off.** Only
...
10. **`--json` on its own is refused.** `validate_args` demands a
...
11. **A symbol documented without parameters is never checked.** The gate in
...
* The README says checking is skipped for "symbols with no doc comment at
  all". The gate is a symbol with no *parameter* documented — see bug 11.
