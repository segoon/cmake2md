#[==[.rst:
@file
@brief Runs cmake2doc from a CMake build.
...
cmake2doc_generate(TARGET docs)
```

What to render, where to write it and which files to read are in the
project's `cmake2doc.toml`, so they are written once rather than once here and
once there.

The module is itself documented with cmake2doc's own tags, so it doubles as a
worked example.
#]==]

# @defgroup cmake Using cmake2doc from CMake
#
...
cmake2doc is run from the current source directory, and reads the nearest
`cmake2doc.toml` at or above it. Everything it does is said there; a target
...
cmake2doc_generate(TARGET docs)
#]==]
function(cmake2doc_generate)
    cmake_parse_arguments(ARG "ALL" "TARGET" "" ${ARGN})

    if(NOT ARG_TARGET)
        message(FATAL_ERROR "cmake2doc_generate: TARGET is required")
    endif()

    if(NOT CMAKE2MD_EXECUTABLE)
        find_program(CMAKE2MD_EXECUTABLE cmake2doc)
    endif()
    if(NOT CMAKE2MD_EXECUTABLE)
        message(
            FATAL_ERROR
            "cmake2doc_generate: cmake2doc was not found on PATH. Install it "
            "with 'pip install cmake2doc', or set CMAKE2MD_EXECUTABLE to its "
            "location."
        )
    endif()
    ...
    add_custom_target(
        ...
        # Where cmake2doc.toml is looked for, and what the paths in it are
    ...
    add_custom_target(
        ...
        WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
        COMMENT "cmake2doc: checking ${ARG_TARGET}"
        VERBATIM
    )
endfunction()
